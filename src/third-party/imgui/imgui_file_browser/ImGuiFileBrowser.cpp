...
bool ImGuiFileBrowser::showFileDialog(const std::string& label, const DialogMode mode, const ImVec2& sz_xy, const std::string& validTypes)
{
	dialog_mode = mode;
	...
	for (std::vector<const Info*>::size_type i = 0; i < filtered_dirs.size(); i++)
	{
		if (!filtered_dirs[i]->is_hidden || show_hidden)
		{
			...
			if (ImGui::Selectable(filtered_dirs[i]->name.c_str(), selected_idx == static_cast<int>(i) && is_dir, ImGuiSelectableFlags_AllowDoubleClick))
			{
				...
				// hack to check if double clicked,
				if (selected_idx == static_cast<int>(i))
				{
					show_error |= !(onDirClick((int)i));
					break;
				}
...
#	endif // OSWIN
			...
			{
#	ifdef OSWIN
				std::string strDir = pathdir + std::string(ent->d_name);
				// IF system file skip it...
				if (FILE_ATTRIBUTE_SYSTEM & GetFileAttributesA(strDir.c_str()))
					continue;
				if (FILE_ATTRIBUTE_HIDDEN & GetFileAttributesA(strDir.c_str()))
					is_hidden = true;
#	else
				if (name[0] == '.')
					is_hidden = true;
...
void ImGuiFileBrowser::filterFiles(int filterMode)
{
	filter_dirty = false;
	if (filterMode & FilterMode_Dirs)
	{
		filtered_dirs.clear();
		for (std::vector<Info>::size_type i = 0; i < subdirs.size(); ++i)
		{
	...
	if (filterMode & FilterMode_Files)
	{
		filtered_files.clear();
		for (std::vector<Info>::size_type i = 0; i < subfiles.size(); ++i)
		{
			...
			if (show_all_valid_files)
			{
				...
				{
					std::string strExt = subfiles[i].name.find_last_of('.') == std::string::npos ? "" : subfiles[i].name.substr(subfiles[i].name.find_last_of('.'));
					std::transform(strExt.begin(), strExt.end(), strExt.begin(), [](unsigned char c) { return std::tolower(c); });
					if (strExt.length() > 0 && find(valid_exts.begin(), valid_exts.end(), strExt) != valid_exts.end())
					{
